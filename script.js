...
function evalc() {
  var s = String(document.getElementById('calctext').value.replace('cos','Math.cos').replace('sin','Math.sin').replace('tan','Math.tan'))
  var l = document.getElementById('rad').selectedOptions[0].value;
  if (l === "deg") {
    s = s.replace('(','((').replace(')','))').replace('cos((','cos(Math.PI/180*(').replace('sin((','sin(Math.PI/180*(').replace('tan((','tan(Math.PI/180*(')
  }
  var res = eval(s);
  if (String(res) === "undefined" || String(res) === "NaN") {
    ...
    document.getElementById('calcres').innerHTML=res.toFixed(decim).toLocaleString('fullwide', {useGrouping:false});}
}
function evals() {
  ...
  if (x1/y1 === x2/y2) {
    ...
    else {document.getElementById('simres').innerHTML = "No solutions";}
  }
  else {
